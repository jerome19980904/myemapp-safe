...
export async function goToCache(uri) {
  if (uri) {
    ...
    if (file.exists) {
      console.log("Fichier déjà dans le cache avec le shorthash : " + name);
      return file.uri;
    }

    console.log("Fichier mis dans le cache avec le shorthash : " + name);
    const newFile = await FileSystem.downloadAsync(uri, path);
    return newFile.uri;
  }
}

//nb est un chiffre, pas une string
export function addZeroToNumber(nb) {
  if (nb < 10) {
    nb = "0" + nb;
  }
  return nb;
...
//time est en SECONDES, un chiffre donc
export function convertToDate(time) {
  timeStamp = new Date(time * 1000);
  return (
    timeStamp.getFullYear().toString() +
    "-" +
    addZeroToNumber(timeStamp.getMonth() + 1).toString() +
    "-" +
    addZeroToNumber(timeStamp.getDate()).toString()
  );
}

//time est en SECONDES, un chiffre donc
export function convertToFrenchDate(time, type, firebase) {
  timeStamp = new Date(time * 1000);
  var ajout = 0
  if(firebase){
    ajout = -1969
  ...
  if (type === "monthLetter") {
    return (
      ...
      " " +
      (timeStamp.getFullYear()+ajout).toString()
    );
  } else if (type === "fullLetter") {
    return (
      dayNames[timeStamp.getDay()] +
      " " +
      addZeroToNumber(timeStamp.getDate()).toString() +
      " " +
      monthNames[timeStamp.getMonth()] +
...
//time est en SECONDES, un chiffre donc
export function getTimeFromDate(time, type) {
  timeStamp = new Date(time * 1000);
  var separateur = ":"
  var minutes=addZeroToNumber(timeStamp.getMinutes()).toString()
  if (type==="h"){
    separateur="h"
    if (minutes==="00"){
      minutes=""
  ...
  return (
    addZeroToNumber(timeStamp.getHours()).toString() +
    separateur +
    addZeroToNumber(timeStamp.getMinutes()).toString()
  );
}

//Convertit une uri en fichier Blob
...
//récupère les dimensions de l'appareil, utile pour dimensionner de la bonne façon
export const { width: viewportWidth, height: viewportHeight } = Dimensions.get(
  "window"
);


//Fonction de remplacement des diactriques
...
export function remplace_diacritiques (chaine) {
...
//Constantes qui servent à définir le nom des mois, jours pour les dates
